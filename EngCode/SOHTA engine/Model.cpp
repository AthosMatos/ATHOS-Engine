...
bool Model::LoadObjModel(wstring filename,bool isRHCoordSys,bool computeNormals)
{
    loadTransparency();
                            ...
                            if (checkChar == 'K')
                            {
                                ...
                                if (checkChar == 'd')
                                {
                                    ...
                                    if (!alreadyLoaded)
                                    {
                                        ...
                                        hr = CreateWICTextureFromFile(d3dDevice, fileNamePath.c_str(), nullptr, &tempMeshSRV, 0);
    ...
    vector<light_Vertex> vertices;
    light_Vertex tempVert;
    ...
    d3dDevice->CreateBuffer(&indexBufferDesc, &iinitData, &indexBuff);
    ...
    vertexBufferDesc.ByteWidth = sizeof(light_Vertex) * totalVerts;
    ...
    hr = d3dDevice->CreateBuffer(&vertexBufferDesc, &vertexBufferData, &vertBuff);

    return true;
}

void Model::loadTex(wstring filename)
{
    CreateWICTextureFromFile(d3dDevice, filename.c_str(), nullptr, &Texture, 0);
}

void Model::loadTransparency()
{
...
void Model::Update()
{
    ...
    XMMATRIX Translation = XMMatrixTranslation(5.0f, -1.5f, 0.0f);
...
void Model::Render()
{
    ...
    d3dDevice->CreateSamplerState(&sampDesc, &TexSamplerState);
    ...
    for (int i = 0; i < meshSubsets; ++i)
    {
        ...
        d3dDevCon->IASetIndexBuffer(indexBuff, DXGI_FORMAT_R32_UINT, 0);
        //Set the grounds vertex buffer
        d3dDevCon->IASetVertexBuffers(0, 1, &vertBuff, &stride, &offset);
        ...
        d3dDevCon->PSSetConstantBuffers(1, 1, &cbPerObjectBuffer);
        if (material[subsetMaterialArray[i]].hasTexture)
        {
            d3dDevCon->PSSetShaderResources(0, 1, &meshSRV[material[subsetMaterialArray[i]].texArrayIndex]);
        }
        else
        {
            d3dDevCon->PSSetShaderResources(0, 1, &Texture);
        ...
        if (!material[subsetMaterialArray[i]].transparent)
            d3dDevCon->DrawIndexed(indexDrawAmount, indexStart, 0);
    }

    //Draw our model's TRANSPARENT subsets now

    //Set our blend state
    d3dDevCon->OMSetBlendState(Transparency, NULL, 0xffffffff);
